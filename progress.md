...
- GitHub publish session (2026-09-20): created public repo armaygooser/LedgerProof and pushed `main` using the stored GCM credential (secret scan first); fixed the repo description encoding and pointed the README clone URL at the real repo; GHCR package owner turned out to be the `elsechord` org (shared with Peichen), browser parked on the GitHub login page awaiting the user.
- GHCR unblock session (2026-09-20): user logged into ghcr.io with a read:packages token; default image pulled (digest dd4715f6…), governance rebuilt with `--pull`, three containers healthy, full smoke to `rolled_back` (case 007, 42 records). Package stays private (org policy); demo machines need `docker login ghcr.io` until an org owner makes it public.
- UI fix session (2026-09-20): fixed the approval modal overflowing the viewport at 1280×720 — the full 64-char proposal SHA-256 rendered unwrappable inside a CSS grid; now wraps in a monospace `hash-wrap` span with grid `min-width: 0` hardening. Reproduced, fixed, rebuilt the web container, and browser-verified.
